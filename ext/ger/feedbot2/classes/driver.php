...
namespace ger\feedbot2\classes;
...
class driver
{
	 ...
	 */
	public function __construct(\phpbb\config\config $config,  \phpbb\config\db_text $config_text, \phpbb\user $user, \phpbb\language\language $language, \phpbb\auth\auth $auth, \phpbb\db\driver\driver_interface $db, \phpbb\log\log $log, $phpbb_root_path, $php_ext, $track_table, \phpbb\event\dispatcher $phpbb_dispatcher)
	{
		$this->config = $config;
	...
	{
		$ct = $this->config_text->get('ger_feedbot2_current_state');
		if (empty($ct) || $ct === 'null')
		{
    ...
    private function check_state_parameters()
    {
        ...
        $this->config_text->set('ger_feedbot2_current_state', json_encode($new_state));
    }
	...
	public function fetch_all()
	{
        ...
        $lock = (int) $this->config['feedbot2_locked'];
        if ($lock > 0)
        {
            return 0;
        }
        ...
        if (!$this->config->set_atomic('feedbot2_locked', 0, time(), false))
        {
            return 0;
        }
		foreach($this->current_state as $id => $source)
		...
		$this->config_text->set('ger_feedbot2_current_state', json_encode($this->current_state));
        $this->switch_user($active_user);
        $this->config->set('feedbot2_locked', 0, false);
        return $counter;
	}
    ...
    private function get_content($url, $timeout = 10, $useragent_override = false, $force_file_get_contents = false)
    {
        ...
        else
        {
            ...
            // Disabled: does not work when open_basedir is set.
	...
	private function parse_feed($url, $type, $timeout = 3)
	{
        ...
        if (!$data)
		{
			$this->log->add('critical', $this->user->data['user_id'], $this->user->ip, 'FB2_LOG_FEED_TIMEOUT', time(), array($url . ' (' . $timeout . ' s)'));
			return false;
		}
		else
		{
	...
	private function parse_atom($data, $url)
	{
		...
		foreach($content->entry as $item)
		{
            ...
            *
            * @event ger.feedbot2.parse_atom_append
            ...
            */
            ...
            extract($this->phpbb_dispatcher->trigger_event('ger.feedbot2.parse_atom_append', compact($vars)));      

            // Add it to the list
            $return[] = $append;
		}

		$this->log->add('admin', $this->user->data['user_id'], $this->user->ip, 'FB2_LOG_FEED_FETCHED', time(), array($url));
		return $return;
	}
	...
	private function parse_rdf($data, $url)
	{
		...
		{            
            ...
            *
            * @event ger.feedbot2.parse_rdf_append
            * @var  object item  item as found in source
            * @var  array  append Array of properties to be send to the post_message function
            * @since 1.0.1
            */
            $vars = array('item', 'append');
            extract($this->phpbb_dispatcher->trigger_event('ger.feedbot2.parse_rdf_append', compact($vars)));      
	...
	private function parse_rss($data, $url)
	{
		...
		foreach($content->channel->item as $item)
		{
            ...
            *
            * @event ger.feedbot2.parse_rss_append
            * @var  object item  item as found in source
            * @var  array  append Array of properties to be send to the post_message function
            * @since 1.0.1
            */
            $vars = array('item', 'append');
            extract($this->phpbb_dispatcher->trigger_event('ger.feedbot2.parse_rss_append', compact($vars)));      
	...
	public function fetch_items($items, $source_id)
	{
        ...
        $new_latest = [];
        if (empty($this->current_state[$source_id]['update'])) 
        {
            $new_latest = array(
		...
		{
			if ( (empty($this->current_state[$source_id]['update'])) && $this->is_handled($item, $this->current_state[$source_id]['latest']) )
			{
	...
	private function post_message($rss_item, $source_id)
	{
        ...
        {
            $post_text .= "\n\n" . $this->user->lang('FB2_SOURCE') . ' [url]' .  $rss_item['link'] . '[/url]';
        }
		if (!empty($source['update']))
        {
            return $this->update_post($post_text, $title, $rss_item, $source);
        }
        else
        {
             ...
             *
             * @event ger.feedbot2.submit_post_before
            ...
            extract($this->phpbb_dispatcher->trigger_event('ger.feedbot2.submit_post_before', compact($vars)));
            if ($do_post)
            {
    ...
    /**
     * Update existing post instead of posting only new items
     * @param string $post_text
     * @param string $title
     * @param array $rss_item
     * @param array $source
     */
    private function update_post($post_text, $title, $rss_item, $source)
    {
        // Decide on unique item
        if (empty($rss_item['guid']))
		{
            $this->log->add('critical', $this->user->data['user_id'], $this->user->ip, 'FB2_LOG_UPDATE_NO_GUID', time(), array($source['url']));
            return false;
		}
        $feed64 = base64_encode($source['url']);

        
        // Check for current post id if available
        $sql = 'SELECT p.post_id, p.topic_id FROM ' . $this->track_table . ' t, ' . POSTS_TABLE . ' p '
            . ' WHERE feed64 = "' . $feed64 . '" '
            . ' AND guid = "' . $rss_item['guid'] . '" '
        ...
        if (!empty($entry['post_id']))
        {
            ...
            $data = array(
                // General Posting Settings
                'forum_id'			 => $source['forum_id'], // The forum ID in which the post will be placed. (int)
                'topic_id'			 => $entry['topic_id'], // Post a new topic or in an existing one? Set to 0 to create a new one, if not, specify your topic ID here instead.
                'post_id'       	 => $entry['post_id'],
                'poster_id'       	 => $source['user_id'],
                'drop_post_stat'     => true, // Prevent useless counter of edited by ... at ... line
                'icon_id'			 => false, // The Icon ID in which the post will be displayed with on the viewforum, set to false for icon_id. (int)
                // Defining Post Options
                'enable_bbcode'		 => true, // Enable BBcode in this post. (bool)
                'enable_smilies'	 => true, // Enabe smilies in this post. (bool)
            ...
            return submit_post('edit', $title, $this->user->data['username'], POST_NORMAL, $poll, $data, true, true);
        }
        else
        {
            // Post new
            ...
            $post_url = submit_post('post', $title, $this->user->data['username'], POST_NORMAL, $poll, $data);
            if (strpos($post_url, 'sid=') !== false)
            {
                $post_url = substr($post_url, 0, strpos($post_url, 'sid='));
            }
            $topic_id = str_replace('&amp;t=', '', strstr($post_url, '&amp;t='));
            
            // Get post id for topic
            $sql = 'SELECT post_id FROM ' . POSTS_TABLE . ' WHERE topic_id = ' . (int)  $topic_id . ' ORDER BY post_id ASC';
            $result = $this->db->sql_query($sql);
            $post = $this->db->sql_fetchrow($result);
            $this->db->sql_freeresult($result);
            $post_id = (int) $post['post_id'];
            
            // Store post id in track table
            $track = ['feed64' => $feed64, 'guid' => $rss_item['guid'], 'post_id' => $post_id];
            $action = 'INSERT INTO ' . $this->track_table . ' ' . $this->db->sql_build_array('INSERT', $track);
    ...
    /**
     * Unsets tracking if any
     * @param string $url
     * @return bool
     */
    public function unset_tracking($url)
    {
        $feed64 = base64_encode($url);
        $action = 'DELETE FROM ' . $this->track_table . ' WHERE feed64 = "' . $feed64 .'"';
        return $this->db->sql_query($action);
    }
	...
	private function switch_user($new_user_id)
	{
        ...
        {
            $this->language->add_lang('info_acp_feedbot2', 'ger/feedbot2');
            return true;
        }
	...
	{ 
        ...
        * Modify the fetched RSS item before it's added to the return list
        *
        * @event ger.feedbot2.html2bbcode_convert
        * @var  array   $convert  regex array
        * @var  string  $html_string input string
        * @since 1.0.12
        */
        $vars = array('item', 'append');
        extract($this->phpbb_dispatcher->trigger_event('ger.feedbot2.html2bbcode_convert', compact($vars)));   
    ...
    private function log_xml_error($url)
    {
        ...
        $this->log->add('critical', $this->user->data['user_id'], $this->user->ip, 'FB2_LOG_FEED_ERROR', time(), array($url, $xml_errors));
